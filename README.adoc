...
The function will accept data 'GNIRPS', and send it's revers 
to the reverse-in request queue.  By specifying `--reply` in the
riff command, we are requesting that riff await for return data.